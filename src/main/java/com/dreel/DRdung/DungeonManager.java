...
public class DungeonManager {
    ...
    public boolean pasteRandomDungeon(World world, int x, int y, int z) {
        ...
        if (!schematicFile.exists()) {
            Bukkit.getLogger().warning("[DRdung] ОШИБКА: Файл схематики не найден -> " + schematicFile.getAbsolutePath());
            return false;
        }

        ClipboardFormat format = ClipboardFormats.findByFile(schematicFile);
        if (format == null) {
            Bukkit.getLogger().warning("[DRdung] ОШИБКА: Неизвестный формат схематики -> " + schemName);
            return false;
        }
